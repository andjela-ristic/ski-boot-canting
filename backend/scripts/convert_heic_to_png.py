...
def convert_heic_to_png() -> None:
    ...
    heic_files = sorted(list(RAW_DIR.glob("*.HEIC")) + list(RAW_DIR.glob("*.heic")))
    if not heic_files:
    ...
    for heic_path in heic_files:
        ...
        try:
            ...
            metadata.append({"source_file": str(heic_path.relative_to(PROJECT_ROOT)),"output_file": None,"error": str(e),})
...
def save_metadata(metadata: list[dict]) -> None:
    ...
    with open(JSON_PATH, "w", encoding="utf-8") as f: json.dump(metadata, f, indent=2, ensure_ascii=False)
