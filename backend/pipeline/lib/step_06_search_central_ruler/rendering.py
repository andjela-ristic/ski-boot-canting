...
def load_base_edge_image(data: dict) -> tuple[np.ndarray, str | None]:
    ...
    if source_path is not None and source_path.exists():
        ...
        if image is not None: return image, relative_project_path(source_path)

    image_name = data.get("image_name", "")
    for path in [ WORKING_PNG_DIR / image_name, PROCESSED_DIR / "03_edges" / "cleaned" / image_name,]:
        if path.exists():
            image = cv2.imread(str(path), cv2.IMREAD_GRAYSCALE)
            if image is not None: return image, relative_project_path(path)
...
def load_step05_overlay(image_name: str) -> np.ndarray | None:
    ...
    if not overlay_path.exists(): return None
...
def build_fragment_background(base_edge_image: np.ndarray, filtered_lines: list[dict]) -> np.ndarray:
    ...
    for line in filtered_lines: draw_fragment(overlay,line,COLOR_ALL_FRAGMENTS,int(cfg("drawing", "all_fragment_thickness", default=2)),)
    return overlay

def draw_overlay( fragment_background: np.ndarray,  filtered_line_count: int,  best_candidate: dict | None,  fine_candidates: list[dict], roi_profile: dict, image_name: str,) -> np.ndarray:
    ...
    if bool(cfg("drawing", "show_candidate_lines", default=True)):
        for candidate in fine_candidates[: int(cfg("drawing", "candidate_count_to_draw", default=3))]:
            draw_axis(overlay,candidate,roi_profile,COLOR_CANDIDATE,int(cfg("drawing", "candidate_thickness", default=2)),)

    if best_candidate is not None:
        for item in best_candidate.get("selected_support", []): draw_fragment(overlay,item.get("effective_line", item["line"]),COLOR_SELECTED_FRAGMENTS,int(cfg("drawing", "selected_fragment_thickness", default=3)),)
        draw_axis(overlay,best_candidate,roi_profile,COLOR_FINAL_AXIS,int(cfg("drawing", "final_line_thickness", default=4)),)
...
def draw_candidate_snapshot(fragment_background: np.ndarray,candidate: dict,roi_profile: dict,image_name: str,candidate_label: str,) -> np.ndarray:
    overlay = fragment_background.copy()

    for item in candidate.get("selected_support", []): draw_fragment(overlay,item.get("effective_line", item["line"]),COLOR_SELECTED_FRAGMENTS,int(cfg("drawing", "selected_fragment_thickness", default=3)),)
...
def sanitize_candidate(candidate: dict | None) -> dict | None:
    ...
    return {
        ...
        "dense_support_validation_passed": bool(candidate.get("dense_support_validation_passed", False)),
        "sparse_support_validation_passed": bool(candidate.get("sparse_support_validation_passed", False)),
        "support_distribution_passed": bool(candidate.get("support_distribution_passed", False)),
        "support_distribution_mode": candidate.get("support_distribution_mode", "unknown"),
        ...
        "bottom_endpoint_best_fragment_overlap_px": float(candidate.get("bottom_endpoint_best_fragment_overlap_px", 0.0)),
        ...
        "top_original_endpoint_best_fragment_overlap_px": float(candidate.get("top_original_endpoint_best_fragment_overlap_px", 0.0)),
        "bottom_original_endpoint_best_fragment_overlap_px": float(candidate.get("bottom_original_endpoint_best_fragment_overlap_px", 0.0)),
        "top_original_endpoint_fragment_ratio": float(candidate.get("top_original_endpoint_fragment_ratio", 0.0)),
        "bottom_original_endpoint_fragment_ratio": float(candidate.get("bottom_original_endpoint_fragment_ratio", 0.0)),
        ...
        "structural_seed_score": float(candidate.get("structural_seed_score", 0.0)),
        "seed_line_indices": [int(value) for value in candidate.get("seed_line_indices", [])],
        "seed_vertical_separation_ratio": float(candidate.get("seed_vertical_separation_ratio", 0.0)),
        "structural_seed_inside_roi_ratio": float(candidate.get("structural_seed_inside_roi_ratio", 0.0)),
        "roi_prior_x_offset_ratio": candidate.get("roi_prior_x_offset_ratio"),
        "roi_prior_angle_offset_deg": candidate.get("roi_prior_angle_offset_deg"),
        "hypothesis_band_half_width_px": candidate.get("hypothesis_band_half_width_px"),
        "hypothesis_max_angle_error_deg": candidate.get("hypothesis_max_angle_error_deg"),
        ...
        "selected_support": [
            {
                ...
                "source_line_indices": [int(value) for value in item["line"].get("source_line_indices", [item["line"]["line_index"]])],
