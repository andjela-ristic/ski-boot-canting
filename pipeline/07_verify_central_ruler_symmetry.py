...
from lib import step_07_verify_central_ruler_symmetry_lib as step07_lib

def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Step 07: verify Step 06 central-axis candidates using segmented "
            "bidirectional mirror symmetry."
        )
    )
    parser.add_argument(
        "--image",
        type=str,
        default=None,
        help="Optional image-name filter, for example IMG_0502.png",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional processed output-subdir override.",
    )
    parser.add_argument(
        "--segments",
        type=int,
        default=None,
        help="Override the number of vertical verification segments. Default: 12.",
    )
    parser.add_argument(
        "--candidate-limit",
        type=int,
        default=None,
        help="Maximum number of Step 06 top candidates to verify.",
    )
...
def main() -> None:
    ...
    step07_lib.context.STEP_CONFIG = step07_lib.apply_preset(
        step07_lib.context.STEP_CONFIG,
        args.preset,
    )
    if args.output_subdir:
        step07_lib.context.STEP_CONFIG["output_subdir"] = args.output_subdir
    if args.segments is not None:
        if args.segments < 3:
            raise ValueError("--segments must be at least 3")
        step07_lib.context.STEP_CONFIG["segment_count"] = int(args.segments)
    if args.candidate_limit is not None:
        if args.candidate_limit < 1:
            raise ValueError("--candidate-limit must be at least 1")
        step07_lib.context.STEP_CONFIG["candidate_limit"] = int(args.candidate_limit)

    step07_lib.ensure_dirs(
        cleanup=(
            bool(step07_lib.context.STEP_CONFIG.get("cleanup_output_on_start", True))
    ...
    for metadata_path in metadata_files:
        ...
        try:
            ...
            if args.show:
                step07_lib.show_image(
                    step07_lib.PROJECT_ROOT / result["comparison_path"]
                )
